...
/*
unsigned char AllKeyNoChk(unsigned int pt,unsigned char srcflr)
{

    unsigned char i,j,k,l=0;
    unsigned char tmpkey[4];
    
    tmpkey[0]=RcvBuf[pt + SL_mCarKey1+0];
    tmpkey[1]=RcvBuf[pt + SL_mCarKey1+1];
    tmpkey[2]=RcvBuf[pt + SL_mCarKey1+2];
    tmpkey[3]=RcvBuf[pt + SL_mCarKey1+3];
...
/*
	if(Auto && Open && !CarMove){
		if((oldscan[3] & 0x80) && (!bBefK)){
			bDoorOpenWaitOn= !bDoorOpenWaitOn;
			bBefK=1;
		}
