...
#else
    __CONFIG(1, (HSPLL  & FCMDIS & IESODIS & 0xcfff));                          // HSPLL oscillator
    __CONFIG(2, (PWRTEN & BOREN  & BORV28 & WDTEN & WDTPS64 & 0x1f1f));         // PWRTEN disabled, BOR enabled @ 2.0V, HW WD disabled, 1:128 prescaler
    __CONFIG(3, (PBADDIS & LPT1DIS & MCLREN & 0x86ff));                         // PORTB digital on RESET
    __CONFIG(4, (STVREN & LVPDIS & BBSIZ1K & XINSTDIS & DEBUGDIS & 0xffd5));    
                                                                                // DEBUG disabled,
                                                                                // XINST disabled
                                                                                // LVP disabled
                                                                                // STVREN enabled
    __CONFIG(5, (UNPROTECT & 0xc00f));                                          // code unprotect
    __CONFIG(6, (WRTEN & 0xe00f));                                              // code unprotect
    __CONFIG(7, (TREN & 0x400f));                                               // code unprotect
...
// ADCON0 Register...(not use)  
  	CHS3=1;	    // channel select
  	CHS2=1;	    // channel select
  	CHS1=1;
  	CHS0=1;
...
// ADCON1 Register....(not use)
  	PCFG3=1;	    // AD port config bits....(all digital)
  	PCFG2=1;
  	PCFG1=1;
  	PCFG0=1;
  	VCFG0=0; 
