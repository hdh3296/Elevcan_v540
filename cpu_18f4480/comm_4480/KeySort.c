...
/*
	if(UpKeyReadyChk(id)==0){
		UpKeyCalu[id] = 100;
/////////////////////////////////////////////////////////////////////////////////////////////////		return(1); //140509 È¦·£ÅÍ ²¨Áö´Â °Å ¼öÁ¤
	}
*/
...
/*
	...
	if(UP_KEY_LAMP || DN_KEY_LAMP){		
		HALL_LAMP_UP=1;
	}
	else{
		if(HALL_LAMP_UP){
			if(!Open)	HALL_LAMP_UP=0;		
		}
	}                 
*/
...
bHALL_LAMP_UP=0;
HALL_LAMP_DN=0;
if(destfl1 == (MyAddress-1)){
	if(MyFlrCarUse((MyAddress-1),IdPt)){
		if(CarMove)
			bHALL_LAMP_UP=1;
	}
}
...
unsigned char DoorOpenCheck(unsigned char i)
{
...
//	if(bParkingCmdValid)	HostCallMe=1;		


/*
///////////////////
#define		HIB_PARKING_BIT    2
		if(offset_flr == 0){
			if( bSlaveParking && !Parking && Auto){
				CanKeyValue[2] = (CanKeyValue[2] | HIB_PARKING_BIT);
				HostCallMe = 1;
				SlavePrkCounter=0;
			}
			else if( !bSlaveParking && Parking ){
				if(SlavePrkCounter < 20){
					CanKeyValue[2] = (CanKeyValue[2] & ~HIB_PARKING_BIT);
					HostCallMe = 1;
				}
			}
