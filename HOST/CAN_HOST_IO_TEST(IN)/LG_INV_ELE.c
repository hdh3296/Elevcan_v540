...
void  CarCurFloorRead_IO(void)
{
    ...
    if(bCarUpMove){
        if(!IN_SU1){
            if((CurMeterPerMin/10) > cF_VELOCITY60)   bCarErr=1;
        ...
        if(!IN_SU3){
            if((CurMeterPerMin/10) > cF_VELOCITY120)   bCarErr=1;
        ...
        if(IN_UND){
            ...
            if(sRamDArry[mcurfloor] < cF_TOPFLR){
                ...
                else                            sRamDArry[mReqStopFloor]  = 0x0;    
            }
        ...
        if((unsigned long)CurPulse <= (unsigned long)decrease_pulse ){
            ...
            if(OUT_DAC || (sRamDArry[mcurfloor]==0)){   //yousangyong
                IN_UND=1;
                CarLowSpeedCmd_IO();
            }
            else{
