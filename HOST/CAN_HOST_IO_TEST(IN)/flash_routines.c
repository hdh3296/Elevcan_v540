...
void erase_flash_row(unsigned long );
void program_flash(unsigned long, unsigned int);


void erase_flash_row( unsigned long address ) 
{  
	unsigned int Upper_Addr;
	unsigned int Lower_Addr ;

	Upper_Addr = ( address& 0xFFFF0000 )>>16 ;
	Lower_Addr = ( address& 0x0000FFFF ) ;
...
// Note that the top 8 bytes of program memory are not used and
// are set to zero.
void program_flash(unsigned long destination, unsigned int values)
{
	unsigned int Upper_Addr ;
	unsigned int Lower_Addr ;
	Upper_Addr = ( destination& 0xFFFF0000 )>>16 ;
	Lower_Addr = ( destination& 0x0000FFFF ) ;
	// Set up control register to enable programming of 
	// 4 program instructions
	asm("mov		#0x4001,W3");
	asm("mov		W3,NVMCON");
