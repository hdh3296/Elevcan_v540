...
unsigned char  EEPROM_write_Ready(void)
{
    return(WR_START); 
}
...
unsigned char  EEPROM_write_Enable(void)
{
   WREN_ON=1;
   return(0);
}
...
unsigned char  EEPROM_write_Disable(void)
{
   WREN_ON=0;
   return(0);
}
...
void startWriting( unsigned int Eep_Command )
{
...
//DISABLE_INTERRUPTS;
    ...
    while(WR_START){  // && (EepTime < 3) && (EWrTime < 0x0fff) ){
//        EWrTime++;
//        asm("CLRWDT");  
    };
...
void EraseEepromWard( unsigned int Eep_Addr )
{
...
void WriteDataEepromWard( unsigned int Eep_Addr ,unsigned int Eep_Data )
{ 
...
unsigned int EEPROM_Read_Word(unsigned int Eep_Addr)
{
    unsigned int value;
/*
	TBLPAG = EEP_PAGE_ADDR;
...
unsigned int  EEPROM_Write_Word(unsigned int Eep_Adr,unsigned int Eep_Data)
{
...
unsigned int  EEPROM_Write_Byte(unsigned int Eep_Adr,unsigned int Eep_Data)
{
